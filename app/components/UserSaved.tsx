...
const UserSaved = ({ ...props }) => {
  ...
  async function getData(initialLoad = false) {
      ...
      try {
        ...
        console.log(response.posts[0].saved_posts)
        const posts = response.posts[0].saved_posts;
        if (posts.length === 0) {
  ...
  return (
    <FlatList
      ...
      data={data}
      ListEmptyComponent={<Text style={{textAlign : 'center', paddingVertical : 4, backgroundColor :  'white'}}>No Saved Posts</Text>}
      renderItem={({ item }) => (
        <Card
